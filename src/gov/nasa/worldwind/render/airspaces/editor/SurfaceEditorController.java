 ...
 */
public class SurfaceEditorController implements KeyListener, MouseListener, MouseMotionListener
{
    private boolean active;
    private String activeAction;
    private SurfaceEditor editor; // Can be null
    private WorldWindow wwd; // Can be null
    // Current selection and device state.
    private Point mousePoint;
    private SurfaceShape activeShape;
    private SurfaceControlPoint activeControlPoint;
    // Action/Cursor pairings.
    private Map<String, Cursor> actionCursorMap = new HashMap<String, Cursor>();

    protected static final String MOVE_SURFACE_LATERALLY = "SurfaceEdiorController.MoveSurfaceLaterally";
    protected static final String RESIZE_SURFACE= "SurfaceEdiorController.ResizeAirspace";
    protected static final String ADD_CONTROL_POINT = "SurfaceEdiorController.AddControlPoint";
    protected static final String REMOVE_CONTROL_POINT = "SurfaceEdiorController.RemoveControlPoint";
    protected static final String MOVE_CONTROL_POINT = "SurfaceEdiorController.MoveControlPoint";
    ...
    public SurfaceEditorController(WorldWindow wwd)
    {
        this.active = false;
    ...
    public SurfaceEditorController()
    {
        this(null);
    }
    ...
    public void setEditor(SurfaceEditor editor)
    {
        this.editor = editor;
    }
    ...
    protected SurfaceControlPoint getActiveControlPoint()
    {
        return this.activeControlPoint;
    }

    protected void setActiveControlPoint(SurfaceControlPoint controlPoint)
    {
        this.activeControlPoint = controlPoint;
    }

    protected SurfaceShape getActiveSurface()
    {
        return activeShape;
    }

    protected void setActiveSurface(SurfaceShape surfaceShape)
    {
        this.activeShape = surfaceShape;
    }

    protected SurfaceShape getTopOwnedSurfaceAtCurrentPosition()
    {
        ...
        if (this.getEditor().getSurfaceShape() != obj)
            return null;

        return (SurfaceShape) obj;
    }

    protected SurfaceControlPoint getTopOwnedControlPointAtCurrentPosition()
    {
        // Without an editor, we cannot know if the airspace belongs to us.
        if (this.getEditor() == null)
            return null;
        ...
        Object obj = this.getTopPickedObject();
        if (!(obj instanceof SurfaceControlPoint))
            return null;
        ...
        if (this.getEditor() != (((SurfaceControlPoint) obj).getEditor()))
            return null;

        return (SurfaceControlPoint) obj;
    }
    ...
    public void mouseClicked(MouseEvent e)
    {
        ...
        SurfaceControlPoint topControlPoint = this.getTopOwnedControlPointAtCurrentPosition();
    ...
    public void mousePressed(MouseEvent e)
    {
        ...
        SurfaceShape topSurface = this.getTopOwnedSurfaceAtCurrentPosition();        
        SurfaceControlPoint topControlPoint = this.getTopOwnedControlPointAtCurrentPosition();
        ...
        if (e.getButton() == MouseEvent.BUTTON1)
        {
            ...
            else if (e.isAltDown())
            {
                ...
                if (topControlPoint == null)
                {
                    SurfaceControlPoint p = this.handleControlPointAdded(this.getEditor().getSurfaceShape(), e);
                    if (p != null)
                    {
            ...
            else
            {
                ...
                else if (topSurface != null)
                {
                    this.setActive(true);
                    this.setActiveAction(null); // Don't know what action we'll perform until mouseDragged().
                    this.setActiveSurface(topSurface);
                    e.consume();
                }
            }
        }
    ...
    protected SurfaceControlPoint handleControlPointAdded(SurfaceShape surfaceShape, MouseEvent mouseEvent)
    {
        SurfaceControlPoint controlPoint = this.getEditor().addControlPoint(this.getWorldWindow(), surfaceShape,
            mouseEvent.getPoint());
    ...
    protected void handleControlPointRemoved(SurfaceControlPoint controlPoint, MouseEvent mouseEvent)
    {
    ...
    public void mouseDragged(MouseEvent e)
    {
        ...
        if ((e.getModifiersEx() & MouseEvent.BUTTON1_DOWN_MASK) != 0)
        {
            if (this.isActive())
            {
                ...
                else if (this.getActiveSurface() != null)
                {
                    this.handleSurfaceDragged(this.getActiveSurface(), e, lastMousePoint);
                }
                e.consume();
            }
        }
    ...
    protected void handleControlPointDragged(SurfaceControlPoint controlPoint, MouseEvent e, Point lastMousePoint)
    {
        if (e.isShiftDown())
        {
            this.setActiveAction(RESIZE_SURFACE);
    ...
    protected void handleSurfaceDragged(SurfaceShape surfaceShape, MouseEvent e, Point lastMousePoint)
    {
        
            this.setActiveAction(MOVE_SURFACE_LATERALLY);
            this.getEditor().moveSurfaceLaterally(this.getWorldWindow(), surfaceShape, e.getPoint(), lastMousePoint);
    ...
    protected void setupActionCursorMap()
    {
        ...
        this.getActionCursorMap().put(MOVE_SURFACE_LATERALLY, Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR));
        this.getActionCursorMap().put(RESIZE_SURFACE, Cursor.getPredefinedCursor(Cursor.N_RESIZE_CURSOR));
